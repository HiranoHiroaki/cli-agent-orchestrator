...
def assert_readonly_policy(
    ...
    if not allowed_tools:
        raise ValueError(f"{agent_name}: allowed_tools must be explicitly set for deterministic mode")
    disallowed = sorted(tool for tool in allowed_tools if tool not in READ_ONLY_ALLOWED_TOOLS)
    if disallowed:
        raise ValueError(f"{agent_name}: disallowed tools for read-only mode: {','.join(disallowed)}")
